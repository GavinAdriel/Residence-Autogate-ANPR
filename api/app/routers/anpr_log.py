...
def list_anpr_logs(
    ...
    end_date: Optional[date] = Query(
        default=None,
        description="Tanggal akhir (YYYY-MM-DD), inklusif (mencakup seluruh hari itu).",
    ),
    limit: int = Query(default=100, ge=1, le=1000),
    skip: int = Query(default=0, ge=0),
    db: Session = Depends(get_db),
):
    """Log ANPR terbaru lebih dulu, dengan filter opsional (semua filter yang
    diisi digabung dengan AND — misal plate + rentang tanggal bisa dipakai bersamaan).

    Contoh: `/anpr-logs?plate=B1234&start_date=2026-09-01&end_date=2026-09-19`
    """
